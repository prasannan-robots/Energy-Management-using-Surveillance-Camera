...
BLYNK_WRITE(V1) { detection = param.asInt();}
...
void classifier_task(void *pvParameters) {
    ...
    while (1) {
      ...
      Serial.println(detection);
       if (xQueueReceive(frame_queue, &snapshot_buf, portMAX_DELAY) == pdTRUE && detection==1) {
...
void setup() {
  ...
  Serial.setDebugOutput(true);
    ...
    // Optionally add a timeout or a small delay
    delay(100);
...
startCameraServer();
  ...
  String ip = WiFi.localIP().toString(); // Get the local IP address as a String
  String url = "http://" + ip + ":81/stream"; // Concatenate the strings

  Blynk.virtualWrite(V4, url);
  Serial.print(ip);
